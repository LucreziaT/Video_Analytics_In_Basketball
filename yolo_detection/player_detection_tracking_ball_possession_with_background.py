...
outvideo = cv2.VideoWriter('output3.mp4',cv2.VideoWriter_fourcc('M','P','4','V'), 25, (frame_w,frame_h))
ret, frame = cap.read()
...
while True and numero!= 2241:
    ...
    #frame=Transformation(frame,background,kernel1,kernel2,pts1) # i don't know why it works so worse with the background subtraction!
    solocampo = createMask(frame, pts1)
    ...
    for out in outs:
        for detection in out:
            ...
            if confidence > 0.5:
                ...
                if class_id==0: #only class "person" is considered and showed in the output video
...
print('AVPLAYERS: ',int(countertotale/numero))
cap.release()
outvideo.release()
